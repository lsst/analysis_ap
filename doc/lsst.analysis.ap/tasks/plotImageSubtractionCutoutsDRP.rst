...
Note: The code differs from its general implementation when using DRP outputs since you are required to
iterate through each of the data references individually. You must also use the diaSources from the
`goodSeeingDiff_diaSrcTable` which has the required dataframe format for this task.
